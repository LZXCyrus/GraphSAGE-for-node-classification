...
#Pooling
			...
			for feat in [embed_matrix[x.squeeze()] for x in indexs]:
				if len(feat.size()) == 1:
					...
					aggregate_feats.append(torch.mean(feat,0,keepdim=True)[0].view(1, -1))
			aggregate_feats = torch.cat(aggregate_feats, 0)
		elif self.agg_func == 'KMAXPOOL':
			#print(mask)
			indexs = [x.nonzero() for x in mask==1]
#			print(indexs)
#			print(len(indexs))
			...
			for feat in [embed_matrix[x.squeeze()] for x in indexs]:
				if len(feat.size()) == 1:
					...
					firstmax = torch.max(feat,0)[0].view(1, -1)
#					feat1 = torch.max(feat,0)[0]
#					print(feat1.shape)
					mask = (feat == feat.max(dim=0, keepdim=True)[0]).to(dtype=torch.int32)
					mask = mask-1
					mask = mask*(-1)
#					print(mask)
					result = torch.mul(mask, feat)

#					print('zheges:',result.shape)
					secondmax = torch.max(result,0)[0].view(1, -1)
#					print(secondmax)
#					print(firstmax)
					maxx = (firstmax+secondmax)/2
					aggregate_feats.append(maxx)
			aggregate_feats = torch.cat(aggregate_feats, 0)
		# self.dc.logger.info('6')
		elif self.agg_func == 'MAXPOOL+MEANPOOL/2':
			# print(mask)
			indexs = [x.nonzero() for x in mask==1]
			aggregate_feats = []
			# self.dc.logger.info('5')
			for feat in [embed_matrix[x.squeeze()] for x in indexs]:
				if len(feat.size()) == 1:
					...
					aggregate_feats.append((torch.mean(feat,0,keepdim=True)[0].view(1, -1)+torch.max(feat,0)[0].view(1, -1))/2)
			aggregate_feats = torch.cat(aggregate_feats, 0)
			...
			for feat in [embed_matrix[x.squeeze()] for x in indexs]:
				if len(feat.size()) == 1:
					...
					aggregate_feats.append(torch.sum(feat,0,keepdim=True)[0].view(1, -1))
			aggregate_feats = torch.cat(aggregate_feats, 0)

		elif self.agg_func == 'VAR':
			# print(mask)
			indexs = [x.nonzero() for x in mask==1]
			aggregate_feats = []
			for feat in [embed_matrix[x.squeeze()] for x in indexs]:
...
#					print(feat.shape)
					a=torch.var(feat,0,keepdim=True)[0].view(1, -1)
#					print('hello:',a.shape)
					aggregate_feats.append(torch.var(feat,0,keepdim=True)[0].view(1, -1))
			aggregate_feats = torch.cat(aggregate_feats, 0)
			...
			for feat in [embed_matrix[x.squeeze()] for x in indexs]:
				...
				if len(feat.size()) == 1:
					aggregate_feats.append(feat.view(1, -1))
#					print('BBBB:',feat.view(1, -1).shape)
				else:
#					print('aaaa:',feat.shape)
					a=feat.view(1, -1)
#					print(list(feat.size()))
					lstm = nn.LSTM(feat.size(1), feat.size(1), num_layers=1,batch_first=True)
					x, (hn, cn) = lstm(feat)
#					print(x.shape)
					x = x[-1,:]
					x=torch.unsqueeze(x, dim=0)
#					print('怎么说：',x.shape)
					aggregate_feats.append(x)
			aggregate_feats = torch.cat(aggregate_feats, 0)
...
#					print('aaaa:',feat.shape)
					a=feat.view(1, -1)
#					print(list(feat.size()))
					lstm = nn.LSTM(feat.size(1), feat.size(1), num_layers=2,batch_first=True)
					x, (hn, cn) = lstm(feat)
#					print(x.shape)
					x = x[-1,:]
					x=torch.unsqueeze(x, dim=0)
